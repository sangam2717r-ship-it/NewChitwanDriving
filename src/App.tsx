...
import {
  Shield, Lock, Calendar as CalIcon, Clock, Settings, Check, ChevronRight,
  ChevronLeft, Car, MapPin, Phone, MessageCircle, User, FileText, RefreshCcw,
  Eye, EyeOff, Menu, X, Users, Award, Edit3, Trash2, CheckCircle, AlertTriangle,
  Plus, Minus, Smartphone, AlertOctagon, Map, Key, Mail, Save, Search, Globe,
  XCircle, BookOpen, GraduationCap, History, PlayCircle, HelpCircle, ArrowLeft,
  Info, LogOut,
} from "lucide-react";
...
const t = {
  en: {
    ...
    daily_len: "Daily Session Length",
    ...
    safety_title: "Why Formal Training Saves Lives",
    safety_desc: "In Nepal, road accidents are a major national concern. Statistics from the Traffic Directorate show that over 80% of accidents involve two-wheelers. A significant number of these tragic incidents are caused by unlicensed riders or those who skipped formal training. Learning the mechanics is not enough; understanding road discipline, defensive driving, and traffic signs is vital to saving lives. At New Chitwan, we don't just teach you to pass the trial; we teach you to survive on the road.",
    edu_title: "Traffic Signs & Rules",
    edu_subtitle: "Essential knowledge for your Likhit (Written) exam and daily driving.",
    team_title: "Our Team",
    role_proprietor: "Proprietor & Instructor",
    role_manager: "Manager & Reception",
    addr_title: "Address",
    addr_desc: "Bharatpur Height, Chitwan (Same building as Eye Express)",
    ...
    days: "days",
    continue: "Continue",
    signs_mandatory: "Mandatory Signs (Regulatory)",
    signs_warning: "Warning Signs (Cautionary)",
    signs_info: "Informational Signs",
    likhit_prep: "DoTM Mock Test",
    likhit_desc: "Attempt our infinite question pool based on the official 500 Question Collection.",
    start_quiz: "Start Mock Test",
    practice_mode: "Practice Mode (Infinite)",
  ...
  np: {
    ...
    nav_edu: "ट्राफिक संकेत",
    nav_likhit: "लिखित परीक्षा",
    ...
    daily_len: "दैनिक प्रशिक्षण समय",
    est_total: "अनुमानित जम्मा",
    next: "अर्को",
    back: "पछाडि",
    ...
    safety_desc: "नेपालमा सडक दुर्घटना एक प्रमुख समस्या हो। ट्राफिक निर्देशनालयको तथ्यांक अनुसार ८०% भन्दा बढी दुर्घटनामा दुई पांग्रे सवारी साधन संलग्न हुन्छन्। यसको मुख्य कारण अदक्ष र लाइसेन्स नभएका चालकहरू हुन्। मेकानिक्स मात्र जानेर पुग्दैन; सडक अनुशासन र ट्राफिक संकेत बुझ्नु जीवन बचाउनको लागि अपरिहार्य छ। न्यू चितवनमा हामी लाइसेन्स मात्र होइन, सुरक्षित ड्राइभिङ सिकाउँछौं।",
    edu_title: "ट्राफिक संकेत र नियमहरू",
    edu_subtitle: "लिखित परीक्षा र दैनिक जीवनका लागि आवश्यक जानकारी।",
    team_title: "हाम्रो टिम",
    role_proprietor: "प्रोप्राइटर र प्रशिक्षक",
    role_manager: "प्रबन्धक र रिसेप्सन",
    addr_title: "ठेगाना",
    addr_desc: "भरतपुर हाइट, चितवन (आई एक्सप्रेस भएको भवन)",
    ...
    signs_mandatory: "अनिवार्य संकेतहरू (नियम)",
    signs_warning: "चेतावनी संकेतहरू (खतरा)",
    signs_info: "जानकारीमूलक संकेतहरू",
    likhit_prep: "लिखित परीक्षा (Mock Test)",
    likhit_desc: "यातायात व्यवस्था विभागको ५०० प्रश्न सँगालोबाट अभ्यास गर्नुहोस्।",
    start_quiz: "परीक्षा सुरु गर्नुहोस्",
    practice_mode: "अभ्यास मोड (अनन्त)",
...
const quizQuestions = [
  ...
  { id: 3, q: { en: "What is the color of the number plate for private vehicles?", np: "निजी सवारी साधनको नम्बर प्लेटको रङ कस्तो हुन्छ?" }, options: [{ en: "Black with White letters", np: "कालोमा सेतो अक्षर" }, { en: "White with Red letters", np: "सेतोमा रातो अक्षर" }, { en: "Red with White letters", np: "रातोमा सेतो अक्षर" }, { en: "Green with White letters", np: "हरियोमा सेतो अक्षर" }], correct: 2 },
...
const useCopyProtection = (active = true) => {
  useEffect(() => {
    ...
    const preventKeys = (e: any) => { if ((e.ctrlKey || e.metaKey) && ["c", "s", "p", "u", "a"].includes(e.key.toLowerCase())) { e.preventDefault(); } if (e.key === "F12") e.preventDefault(); };
...
const CustomCalendar = ({ schedule, setSchedule, targetDays }: { schedule: Schedule; setSchedule: any; targetDays: number; }) => {
  ...
  const handleDateClick = (day: number) => {
    ...
    if (schedule[dateString]) {
      ...
      if (Object.keys(schedule).length >= targetDays) { alert(`Limit reached: ${targetDays} days.`); return; }
      setPickingDate(dateString);
    }
  };
  const handleTimeSelect = (time: string) => {
    if (pickingDate) { setSchedule({ ...schedule, [pickingDate]: time }); setPickingDate(null); }
  };
  ...
  return (
    <div className="flex flex-col items-center mb-4">
      {type === "mandatory" && (
        <div className="w-20 h-20 relative">
          <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
            <circle cx="50" cy="50" r="45" fill="white" stroke="#DC2626" strokeWidth="12" />
            {icon === "STOP" && <text x="50" y="55" fontSize="28" fontWeight="bold" textAnchor="middle" fill="black">STOP</text>}
            {icon === "NO_ENTRY" && <path d="M30 50 L70 50" stroke="black" strokeWidth="10" />}
            {icon === "NO_PARKING" && <><text x="50" y="60" fontSize="40" fontWeight="bold" textAnchor="middle" fill="black">P</text><line x1="25" y1="75" x2="75" y2="25" stroke="#DC2626" strokeWidth="8" /></>}
            {icon === "NO_HORN" && <><text x="50" y="60" fontSize="30" textAnchor="middle">🎺</text><line x1="25" y1="75" x2="75" y2="25" stroke="#DC2626" strokeWidth="8" /></>}
            {icon === "40" && <text x="50" y="60" fontSize="35" fontWeight="bold" textAnchor="middle" fill="black">40</text>}
            {icon === "LEFT" && <path d="M60 30 L35 50 L60 70 M35 50 L75 50" stroke="black" strokeWidth="8" fill="none" strokeLinecap="round" strokeLinejoin="round" />}
          </svg>
        </div>
      )}
      {type === "warning" && (
        <div className="w-20 h-20 relative flex items-center justify-center">
          <svg viewBox="0 0 100 90" className="w-full h-full drop-shadow-md">
            <path d="M50 5 L95 85 L5 85 Z" fill="white" stroke="#DC2626" strokeWidth="8" strokeLinejoin="round" />
            <g transform="translate(50, 55)">
              {icon === "CROSS" && <text x="0" y="10" fontSize="35" textAnchor="middle">🦓</text>}
              {icon === "LIGHT" && <text x="0" y="10" fontSize="35" textAnchor="middle">🚦</text>}
              {icon === "SCHOOL" && <text x="0" y="10" fontSize="35" textAnchor="middle">🚸</text>}
              {icon === "BEND" && <path d="M10 20 L-10 20 L-10 -10" stroke="black" strokeWidth="6" fill="none" transform="translate(0,-5)" />}
              {icon === "HILL" && <path d="M-20 20 L20 -10" stroke="black" strokeWidth="6" fill="none" />}
              {icon === "HAZARD" && <text x="0" y="5" fontSize="40" fontWeight="bold" textAnchor="middle" fill="black">!</text>}
            </g>
          </svg>
        </div>
      )}
      {type === "info" && (
        <div className="w-20 h-20 relative">
          <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-md">
            <rect x="5" y="5" width="90" height="90" rx="10" fill="#2563EB" stroke="white" strokeWidth="4" />
            {icon === "P" && <text x="50" y="65" fontSize="50" fontWeight="bold" textAnchor="middle" fill="white">P</text>}
            {icon === "H" && <text x="50" y="65" fontSize="50" fontWeight="bold" textAnchor="middle" fill="white">H</text>}
            {icon === "FUEL" && <text x="50" y="65" fontSize="40" textAnchor="middle">⛽</text>}
            {icon === "TOOL" && <text x="50" y="65" fontSize="40" textAnchor="middle">🔧</text>}
          </svg>
        </div>
      )}
...
const Navbar = ({ setView, activeView, lang, setLang }: any) => {
  ...
  return (
    <nav className="bg-red-900 text-white sticky top-0 z-50 shadow-lg border-b-4 border-red-700 select-none">
      <div className="max-w-6xl mx-auto px-4">
        <div className="flex justify-between items-center h-16">
          <div onClick={() => setView("home")} className="flex items-center gap-3 cursor-pointer">
            ...
            <div>
              <h1 className="font-bold text-lg leading-tight">New Chitwan</h1>
              <p className="text-[10px] text-red-200 uppercase">Driving Training Centre</p>
            </div>
          </div>
          <div className="hidden md:flex items-center space-x-1">
            ...
            {navItems.map((i) => (
              <button key={i.id} onClick={() => setView(i.id)} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${activeView === i.id ? "bg-white text-red-700 font-bold" : "text-red-100 hover:bg-red-800"}`}>
                {i.l}
              </button>
            ))}
            <button onClick={() => setView("login")} className="ml-4 px-3 py-2 bg-red-950 rounded-full hover:bg-black border border-red-800">
      ...
      {isOpen && (
        <div className="md:hidden bg-red-800 border-t border-red-700 p-2">
          {navItems.map((i) => (
            <button key={i.id} onClick={() => { setView(i.id); setIsOpen(false); }} className="block w-full text-left px-3 py-3 rounded-md text-base font-medium text-red-100 hover:bg-red-700">{i.l}</button>
          ))}
...
const QuizView = ({ lang, setView }: any) => {
  ...
  useEffect(() => { if (quizStarted) { setShuffledQuestions([...quizQuestions].sort(() => 0.5 - Math.random())); } }, [quizStarted]);
  ...
  const nextQuestion = () => {
    ...
    if (isPracticeMode && currentQ + 1 >= shuffledQuestions.length) {
      ...
      setCurrentQ(0); setSelectedOption(null);
    } else if (currentQ + 1 < shuffledQuestions.length) {
      setCurrentQ(currentQ + 1); setSelectedOption(null);
    } else {
      setShowResult(true);
    }
  };

  const prevQuestion = () => { if (currentQ > 0) { setCurrentQ(currentQ - 1); setSelectedOption(null); } };
  ...
  return (
    <div className="max-w-lg mx-auto p-4 my-8">
      <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200 relative">
        <button onClick={() => setView("home")} className="absolute top-4 right-4 text-white hover:text-red-200 z-10 font-bold flex items-center gap-1"><LogOut className="w-4 h-4" /> {T.exit}</button>
        <div className="bg-red-900 p-4 flex justify-between items-center text-white pt-10">
          <span className="font-bold">Q. {currentQ + 1}</span>
        ...
        <div className="p-6">
              ...
              return (
                <button key={idx} onClick={() => handleAnswer(idx)} className={btnClass}>
                  <div className="flex justify-between items-center">
                    ...
                    {isPracticeMode && selectedOption === idx && (idx === q.correct ? <CheckCircle className="w-5 h-5 text-green-600" /> : <XCircle className="w-5 h-5 text-red-600" />)}
                  </div>
                </button>
              );
            })}
          ...
          <div className="mt-6 flex gap-2">
            <button onClick={prevQuestion} disabled={currentQ === 0} className="flex-1 py-3 border border-gray-300 rounded-lg font-bold text-gray-500 disabled:opacity-50 flex items-center justify-center gap-1"><ArrowLeft className="w-4 h-4" /> {T.prev}</button>
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 animate-fade-in select-none">
      ...
      <div className="space-y-12">
        <div>
          <h3 className="text-xl font-bold text-red-700 mb-4 border-b pb-2 flex items-center gap-2"><AlertOctagon className="w-5 h-5" /> {T.signs_mandatory}</h3>
          <div className="grid grid-cols-3 md:grid-cols-6 gap-4"><Sign type="mandatory" icon="STOP" label="Stop" /><Sign type="mandatory" icon="NO_ENTRY" label="No Entry" /><Sign type="mandatory" icon="LEFT" label="Left Only" /><Sign type="mandatory" icon="40" label="Speed 40" /><Sign type="mandatory" icon="NO_PARKING" label="No Parking" sub="Restricted" /><Sign type="mandatory" icon="NO_HORN" label="No Horn" sub="Silent Zone" /></div>
        </div>
        <div>
          <h3 className="text-xl font-bold text-yellow-600 mb-4 border-b pb-2 flex items-center gap-2"><AlertTriangle className="w-5 h-5" /> {T.signs_warning}</h3>
          <div className="grid grid-cols-3 md:grid-cols-6 gap-4"><Sign type="warning" icon="LIGHT" label="Traffic Light" /><Sign type="warning" icon="CROSS" label="Zebra Cross" /><Sign type="warning" icon="SCHOOL" label="School" /><Sign type="warning" icon="BEND" label="Left Bend" /><Sign type="warning" icon="HILL" label="Steep Hill" /><Sign type="warning" icon="HAZARD" label="Hazard" /></div>
        </div>
        <div>
          <h3 className="text-xl font-bold text-blue-600 mb-4 border-b pb-2 flex items-center gap-2"><Info className="w-5 h-5" /> {T.signs_info}</h3>
          <div className="grid grid-cols-3 md:grid-cols-6 gap-4"><Sign type="info" icon="P" label="Parking" /><Sign type="info" icon="H" label="Hospital" /><Sign type="info" icon="FUEL" label="Fuel" /><Sign type="info" icon="TOOL" label="Workshop" /></div>
        </div>
      </div>
    </div>
  );
...
const BookingView = ({ onAddBooking, rates, lang }: any) => {
  ...
  useEffect(() => {
    ...
    else base = dailyTime === "60 Mins" ? rates["30 Days"] : rates["30 Days (30m)"];
    ...
    return (
      <div className="min-h-[500px] flex items-center justify-center bg-white rounded-xl shadow-xl p-8 text-center animate-fade-in">
        <div className="max-w-md">
          ...
          <button onClick={() => { setStep("customize"); setDuration("15 Days"); setSchedule({}); }} className="px-6 py-3 bg-red-700 text-white rounded-lg font-bold">{T.nav_book}</button>
        </div>
      </div>
    );
  }
        ...
        <div className="p-8 flex flex-col items-center justify-center h-full">
          ...
          {myBooking && (
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 w-full max-w-md">
              ...
              <div className="text-center p-3 bg-white rounded border border-gray-200 text-sm whitespace-pre-wrap">{myBooking.date}</div>
            </div>
          )}
        </div>
      )}
          ...
          <div className="md:w-1/2 p-6 bg-white">
            ...
            {step === "date" && (
              <div className="animate-fade-in">
                ...
                <div className="mb-4"><CustomCalendar schedule={schedule} setSchedule={setSchedule} targetDays={getTargetDays()} /></div>
                <button onClick={() => { if (Object.keys(schedule).length === 0) alert("Please select at least one date."); else setStep("form"); }} className="mt-2 w-full py-3 bg-red-700 text-white rounded-lg font-bold hover:bg-red-800 flex items-center justify-center gap-2">{T.continue} <ChevronRight className="w-4 h-4" /></button>
                <button onClick={() => setStep("customize")} className="mt-4 text-center text-sm text-gray-400 underline block w-full">{T.back}</button>
              </div>
            )}
            {step === "form" && (
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 animate-fade-in select-none">
      <div className="bg-white rounded-2xl shadow-xl overflow-hidden mb-12">
        ...
        <div className="p-8 md:p-12"><p className="text-lg text-gray-600 leading-relaxed mb-6">{T.history_desc}</p><div className="mt-8 flex items-center gap-2 text-sm font-mono text-gray-400 bg-gray-50 p-3 rounded inline-block"><span>PAN No: 301569099</span></div></div>
      </div>
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12 animate-fade-in select-none">
      ...
      <div className="bg-gray-200 rounded-xl overflow-hidden shadow-inner h-96 w-full relative mt-8">
        <iframe className="absolute inset-0 w-full h-full" src="https://maps.google.com/maps?q=MCQH%2B28+Bharatpur&t=&z=17&ie=UTF8&iwloc=&output=embed" style={{ border: 0 }} allowFullScreen={true} loading="lazy" title="Location Map"></iframe>
      </div>
    </div>
  );
};

const AdminPanel = ({ onExit, rates, setRates }: any) => {
  ...
  const handleSaveBookingEdit = async () => {
    ...
    await updateDoc(doc(db, "artifacts", appId, "public", "data", "bookings", editingBooking.id), {
      clientName: editingBooking.clientName, clientPhone: editingBooking.clientPhone, price: Number(editingBooking.finalPrice), date: dateString, status: editingBooking.status === "pending" ? "approved" : editingBooking.status,
    });
  ...
  const sendTestMessage = (booking: any) => {
    ...
    window.open(`sms:${booking.clientPhone}?body=${encodeURIComponent(msg)}`, "_blank");
  };
  ...
  const handleAddPrivate = async () => {
    ...
    else price = pDaily === "60 Mins" ? rates["30 Days"] : rates["30 Days (30m)"];
    ...
    await addDoc(collection(db, "artifacts", appId, "public", "data", "bookings"), {
      clientName: pName, clientPhone: pPhone, packageName: pkgName, duration: pDuration, dailyTime: pDaily, date: dateString, price: price, instructor: "Prem Bahadur Gaire", type: "private", status: "private", notes: pNotes, progress: 0, createdAt: serverTimestamp(),
    });
            ...
            <div className="space-y-4">
              {activeBookings.length === 0 ? <p className="text-gray-400 italic">No active students.</p> : activeBookings.map((b) => (
                <div key={b.id} className="p-4 border rounded-lg hover:bg-gray-50 relative group">
                  ...
                  {(b.packageName.includes("15") || b.packageName.includes("30")) && (
                    <div className="bg-gray-100 p-3 rounded mb-3">
                      <div className="flex justify-between text-xs font-bold text-gray-500 mb-2"><span>Course Progress</span><span>Day {b.progress || 0}</span></div>
                      <div className="flex items-center gap-2"><button onClick={() => updateProgress(b, -1)} className="p-1 bg-red-100 text-red-600 rounded hover:bg-red-200"><Minus className="w-4 h-4" /></button><div className="flex-grow h-2 bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-green-500 transition-all" style={{ width: `${((b.progress || 0) / (b.packageName.includes("30") ? 30 : 15)) * 100}%` }}></div></div><button onClick={() => updateProgress(b, 1)} className="p-1 bg-green-100 text-green-600 rounded hover:bg-green-200"><Plus className="w-4 h-4" /></button></div>
                    </div>
                  )}
                  <div className="pt-3 border-t flex justify-end gap-2"><a href={`tel:${b.clientPhone}`} className="px-3 py-1 bg-gray-100 text-gray-600 rounded text-xs font-bold hover:bg-gray-200">Call</a><button onClick={() => sendTestMessage(b)} className="px-3 py-1 bg-blue-100 text-blue-600 rounded text-xs font-bold hover:bg-blue-200 flex items-center gap-1"><MessageCircle className="w-3 h-3" /> Msg</button><button onClick={() => sendConfirmation(b)} className="px-3 py-1 bg-[#25D366] text-white rounded text-xs font-bold hover:opacity-90 flex items-center gap-1"><CheckCircle className="w-3 h-3" /> Info</button></div>
                </div>
              ))}
            </div>
          )}
          ...
          {adminTab === "settings" && (
            <div className="max-w-md space-y-6">
              ...
              <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
                ...
                {Object.keys(tempRates).map((key) => (<div key={key} className="mb-2"><label className="text-xs font-bold text-gray-500 block">{key}</label><input type="number" className="w-full p-2 border rounded" value={tempRates[key]} onChange={(e: any) => setTempRates({ ...tempRates, [key]: Number(e.target.value) })} /></div>))}
                <button onClick={() => { setRates(tempRates); alert("Rates Saved!"); }} className="w-full py-2 bg-red-900 text-white rounded text-sm font-bold mt-2">Update Prices</button>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="min-h-[60vh] flex items-center justify-center p-4">
            <div className="w-full max-w-sm bg-white p-8 rounded-xl shadow-xl animate-fade-in border-t-4 border-red-800">
              <div className="text-center mb-6">
                ...
                <p className="text-xs text-gray-400 mt-1">Secure Staff Access</p>
              </div>
              <form onSubmit={handleLogin} className="space-y-4">
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Email</label>
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@newchitwan.com" className="w-full p-3 border rounded-lg outline-none focus:border-red-500" autoFocus />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Password</label>
                  <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="w-full p-3 border rounded-lg outline-none focus:border-red-500" />
                </div>
                {loginError && <div className="p-3 bg-red-50 border border-red-100 rounded text-red-600 text-sm text-center flex items-center justify-center gap-2"><AlertTriangle className="w-4 h-4" /> {loginError}</div>}
                <button type="submit" className="w-full p-3 bg-red-900 text-white rounded-lg font-bold hover:bg-red-800 shadow-md transition-all">Secure Login</button>
              </form>
              <button onClick={() => setView("home")} className="w-full mt-4 text-sm text-gray-400 hover:text-gray-600">Cancel</button>
            </div>
          </div>
        )}
