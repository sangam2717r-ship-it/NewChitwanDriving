...
import { 
  ...
  Edit3, Trash2, CheckCircle, AlertTriangle, Plus, Minus, Smartphone, AlertOctagon, Map, Key, Mail, Save, Search, Globe, XCircle
} from 'lucide-react';
...
const t = {
  en: {
    nav_home: "Home", nav_book: "Book Training", nav_about: "About Us", nav_contact: "Contact", nav_admin: "Admin Login",
    ...
    history_title: "Our History", history_desc: "Serving Bharatpur since April 3rd, 2003. New Chitwan Driving Training Centre was established with a mission to create safe, responsible, and skilled drivers.",
    ...
    footer_rights: "All Rights Reserved", select_date: "Schedule Your Classes", pref_time: "Preferred Time",
  ...
  np: {
    nav_home: "गृहपृष्ठ", nav_book: "तालिम बुकिङ", nav_about: "हाम्रो बारेमा", nav_contact: "सम्पर्क", nav_admin: "एडमिन लगइन",
    ...
    history_title: "हाम्रो इतिहास", history_desc: "२०६० साल देखि चितवनमा सेवारत। सुरक्षित र जिम्मेवार चालक उत्पादन गर्ने हाम्रो मुख्य लक्ष्य हो।",
    ...
    footer_rights: "सर्वाधिकार सुरक्षित", select_date: "कक्षाको समय तालिका", pref_time: "समय छान्नुहोस्",
...
interface Schedule { [date: string]: string } // { "2024-01-01": "7:00 AM" }

const CourseScheduler = ({ schedule, setSchedule, targetDays }: { schedule: Schedule, setSchedule: any, targetDays: number }) => {
    ...
    <div className="select-none">
          ...
          {Array(daysInMonth).fill(null).map((_, i) => {
            ...
            return (
              <div 
                ...
                className={`
                  h-9 w-9 flex items-center justify-center rounded-full text-sm font-medium cursor-pointer transition-all
                  ${isSelected ? 'bg-green-600 text-white shadow-md scale-105 ring-2 ring-green-100' : 'hover:bg-red-50 text-gray-700'}
                `}
              >
                {day}
              </div>
      ...
      {selectedCount > 0 && (
        <div className="bg-gray-50 rounded-lg p-3 max-h-32 overflow-y-auto border border-gray-100">
          <h4 className="text-xs font-bold text-gray-500 mb-2 uppercase">Your Schedule</h4>
          <div className="space-y-1">
            {Object.entries(schedule).sort().map(([date, time]) => (
              <div key={date} className="flex justify-between text-sm bg-white p-2 rounded border border-gray-100 shadow-sm">
...
const Navbar = ({ setView, activeView, lang, setLang }: any) => {
  ...
  const navItems = [{id:'home',l:T.nav_home}, {id:'booking',l:T.nav_book}, {id:'about',l:T.nav_about}, {id:'contact',l:T.nav_contact}];
...
const BookingView = ({ onAddBooking, rates, lang }: any) => {
  ...
  const handleSubmitBooking = async () => {
    ...
    try {
      ...
      await onAddBooking({
        ...
        price: currentPrice, instructor, type: 'public', status: 'pending', progress: 0
      });
  ...
  const handleCheckProgress = async () => {
    ...
    if(snapshot.empty) setCheckError(T.check_not_found); else setMyBooking(snapshot.docs[0].data());
  };
          ...
          <div className="md:w-1/2 p-6 bg-white">
            ...
            {step === 'date' && (
               <div className="animate-fade-in">
                 ...
                 <div className="mb-4">
                   <CustomCalendar schedule={schedule} setSchedule={setSchedule} targetDays={getTargetDays()} />
                 </div>
                 {/* Explicit "Continue" button - No Auto Jump */}
                 <button 
                    onClick={() => {
                        if(Object.keys(schedule).length === 0) alert("Please select at least one date.");
                        else setStep('form');
                    }} 
...
const AdminPanel = ({ securitySettings, updateSecurity, onExit, rates, setRates }: any) => {
  ...
  // PARSE EXISTING SCHEDULE STRING TO OBJECT FOR EDITING
  ...
  const openEdit = (b: any) => {
    setEditingBooking(b);
    setEditSchedule(parseSchedule(b.date)); // Load schedule into state
  };
  ...
  const handleAddPrivate = async () => {
     ...
     await addDoc(collection(db, 'artifacts', appId, 'public', 'data', 'bookings'), { 
         clientName: pName, clientPhone: pPhone, packageName: pkgName, duration: pDuration, dailyTime: pDaily, date: dateString, 
        ...
        <div className="absolute inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-sm shadow-2xl animate-fade-in overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-3">
               ...
               <div>
                 <label className="text-xs text-gray-500 font-bold">Schedule Editor</label>
                 <CustomCalendar schedule={editSchedule} setSchedule={setEditSchedule} targetDays={100} />
               </div>
            </div>
            ...
            <div className="space-y-3">
              {pendingBookings.length === 0 ? <p className="text-gray-400 italic">No new requests.</p> : pendingBookings.map((b) => (
                <div key={b.id} className="p-4 border border-orange-200 bg-orange-50 rounded-lg flex flex-col sm:flex-row justify-between items-start gap-4">
                  <div><p className="font-bold text-gray-800">{b.clientName} <span className="text-xs font-normal text-gray-500">({b.clientPhone})</span></p><p className="text-sm text-gray-600">{b.packageName}</p><div className="text-sm text-orange-600 bg-orange-50 p-2 rounded mt-1 break-words whitespace-pre-wrap">{b.date}</div></div>
               ...
               {activeBookings.length === 0 ? <p className="text-gray-400 italic">No active students.</p> : activeBookings.map((b) => (
                 <div key={b.id} className="p-4 border rounded-lg hover:bg-gray-50 relative group">
                    <div className="flex justify-between items-start mb-2">
                        <div><div className="flex items-center gap-2"><p className="font-bold text-gray-800 text-lg">{b.clientName}</p><button onClick={() => deleteBooking(b.id)} className="text-red-400 hover:text-red-600 text-xs">Remove</button></div><p className="text-sm text-gray-500">{b.packageName}</p>{b.notes && <p className="text-xs text-amber-600 bg-amber-50 p-1 rounded mt-1 inline-block">📝 {b.notes}</p>}<div className="flex items-center gap-2 mt-1"><p className="text-xs font-bold text-blue-600 bg-blue-50 p-1 rounded break-words max-w-xs whitespace-pre-wrap">{b.date}</p><button onClick={() => openEdit({...b, finalPrice: b.price, newDate: b.date})}><Edit3 className="w-3 h-3 text-gray-400 hover:text-blue-500" /></button></div></div>
          ...
          {adminTab === 'private' && (
             <div className="max-w-md space-y-4">
                 ...
                 <div className="border p-2 rounded">
                    ...
                    <CustomCalendar schedule={pSchedule} setSchedule={setPSchedule} targetDays={100} />
                 </div>
