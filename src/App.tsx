...
const t = {
  en: {
    ...
    footer_rights: "All Rights Reserved", select_date: "Select Dates", pref_time: "Preferred Time",
  ...
  np: {
    ...
    footer_rights: "सर्वाधिकार सुरक्षित", select_date: "मिति छान्नुहोस्", pref_time: "समय छान्नुहोस्",
    opt_1day: "१ दिन (ट्रायल)", opt_15days: "१५ दिन", opt_30days: "३० दिन", opt_30mins: "३० मिनेट", opt_60mins: "६० मिनेट",
    selected: "छानिएको", days: "दिन"
  }
};
...
const CustomCalendar = ({ selectedDates, setSelectedDates, maxSelection }: any) => {
  ...
  const handleDateClick = (day: number) => {
    ...
    if (selectedDates.includes(dateString)) {
      newDates = selectedDates.filter((d: string) => d !== dateString); // Deselect
    } else {
  ...
  return (
    <div className="bg-white border rounded-lg p-4 shadow-sm select-none">
          ...
          return (
            <div 
              ...
              className={`
                ...
                ${isSelected ? 'bg-red-600 text-white shadow-md scale-110' : 'hover:bg-red-50 text-gray-700'}
              `}
            >
              {day}
            </div>
      ...
      <div className="mt-3 text-xs text-center text-gray-500 font-medium">
        {selectedDates.length > 0 ? `${selectedDates.length} selected` : 'Select dates'}
      </div>
    </div>
  );
};
...
const BookingView = ({ onAddBooking, rates, lang }: any) => {
  ...
  useEffect(() => {
    ...
    setSelectedDates([]); // Reset dates on duration change
  ...
  const handleSubmitBooking = async () => {
    ...
    try {
      ...
      const dateString = `${selectedDates.join(', ')} @ ${selectedTime || 'Any Time'}`;

      await onAddBooking({
        clientName, clientPhone, packageName: pkgName, duration, dailyTime,
        date: dateString, // Storing all dates
    ...
    return (
      <div className="min-h-[500px] flex items-center justify-center bg-white rounded-xl shadow-xl p-8 text-center animate-fade-in">
        <div className="max-w-md">
          ...
          <button onClick={() => { setStep('customize'); setDuration('15 Days'); setSelectedDates([]); }} className="px-6 py-3 bg-red-700 text-white rounded-lg font-bold">{T.nav_book}</button>
        </div>
      </div>
    );
  }
         ...
         <div className="p-8 flex flex-col items-center justify-center h-full">
            ...
            {myBooking && (
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 w-full max-w-md">
                 ...
                 <div className="text-center p-3 bg-white rounded border border-gray-200 text-sm break-words">{myBooking.date}</div>
              </div>
            )}
         </div>
      )}
               ...
               <div className="animate-fade-in">
                 ...
                 <div className="mb-6">
                   ...
                   <CustomCalendar 
                      ...
                      maxSelection={duration === '1 Day' ? 1 : 100}
                   />
                 </div>
                 {selectedDates.length > 0 && (
                   <div className="animate-fade-in">
                      <h3 className="font-bold text-lg mb-4">{T.pref_time}</h3>
                      <div className="grid grid-cols-3 gap-2">
                        {timeSlots.map(t => (
...
const AdminPanel = ({ securitySettings, updateSecurity, onExit, rates, setRates }: any) => {
  ...
  const [pDates, setPDates] = useState<string[]>([]); // Multi dates for private booking
  ...
  const handleSaveBookingEdit = async () => {
    ...
    // Save dates array or string
    const dateString = editingBooking.dateInput ? `${editingBooking.dateInput} @ ${editingBooking.timeInput || 'Any'}` : editingBooking.newDate;
  ...
  const handleAddPrivate = async () => {
     ...
     const dateString = pDates.length > 0 ? pDates.join(', ') : 'No Dates';
     ...
     alert("Private Booking Added!"); setAdminTab('active'); setPName(''); setPPhone(''); setPNotes(''); setPDates([]);
  };
        ...
        <div className="absolute inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 w-full max-w-sm shadow-2xl animate-fade-in overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-3">
               ...
               <div><label className="text-xs text-gray-500 font-bold">Dates (Manual Text)</label><textarea className="w-full p-2 border rounded h-24" value={editingBooking.newDate} onChange={(e: any) => setEditingBooking({...editingBooking, newDate: e.target.value})}/></div>
            </div>
               ...
               {activeBookings.length === 0 ? <p className="text-gray-400 italic">No active students.</p> : activeBookings.map((b) => (
                 <div key={b.id} className="p-4 border rounded-lg hover:bg-gray-50 relative group">
                    <div className="flex justify-between items-start mb-2">
                        <div><div className="flex items-center gap-2"><p className="font-bold text-gray-800 text-lg">{b.clientName}</p><button onClick={() => deleteBooking(b.id)} className="text-red-400 hover:text-red-600 text-xs">Remove</button></div><p className="text-sm text-gray-500">{b.packageName}</p>{b.notes && <p className="text-xs text-amber-600 bg-amber-50 p-1 rounded mt-1 inline-block">📝 {b.notes}</p>}<div className="flex items-center gap-2 mt-1"><p className="text-xs font-bold text-blue-600 bg-blue-50 p-1 rounded break-words max-w-xs">{b.date}</p><button onClick={() => setEditingBooking({...b, finalPrice: b.price, newDate: b.date})}><Edit3 className="w-3 h-3 text-gray-400 hover:text-blue-500" /></button></div></div>
          ...
          {adminTab === 'private' && (
             <div className="max-w-md space-y-4">
                 ...
                 <div className="border p-2 rounded">
                    ...
                    <CustomCalendar selectedDates={pDates} setSelectedDates={setPDates} maxSelection={100} />
                 </div>
